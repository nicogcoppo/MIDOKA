...
mysql -u "${user}" --password="${pass}" --execute="USE "${DB}";SELECT clID AS 'ID',MAX(IFNULL(COMPLETADO,NOW())) AS 'ULTIMA OPERACION',DATEDIFF(NOW(),MAX(IFNULL(COMPLETADO,NOW()))) AS 'DIAS',DENOMINACION,estaf_lc AS 'LOCALIDAD' FROM OPERACIONES JOIN(CLIENTE,LOCALIDAD) ON (OPERACIONES.INTERESADO=CLIENTE.clID AND CLIENTE.LOCALIDAD=LOCALIDAD.lcID) WHERE SOLICITUD=20 OR SOLICITUD=21 OR SOLICITUD=23 OR SOLICITUD=24 GROUP BY INTERESADO ORDER BY DIAS DESC,cp_lc;" | sed 's/00:00:00//g' | column -t -s $'\t'>${temp}"tmp2.ed"
