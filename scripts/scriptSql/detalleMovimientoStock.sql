USE MIDOKA_PGC_B;
SELECT OPERACIONES.FECHA as 'FECHA',opID AS 'OPERACION',nombre_nm AS 'SOLICITUD',IF(clID=300,IFNULL(NOMBRE_COMERCIAL,'MOV.INTERNO'),DENOMINACION) AS 'INTERESADO',CONCAT(clasificacion_cla," ",mod_md," ",motivo_mt) AS 'ARTICULO',(IFNULL(CANTIDAD_CST,0)+IFNULL(CANTIDAD_ING,0)+IFNULL(CANTIDAD_EGR,0)+IFNULL(CANTIDAD_DEVOL,0)) AS 'CANTIDAD',ROUND(t1.PRECIO*(1 - IFNULL(DESCUENTO, 0) / 100),2) AS 'COSTO',ROUND((IFNULL(CANTIDAD_CST,0)+IFNULL(CANTIDAD_ING,0)+IFNULL(CANTIDAD_EGR,0)+IFNULL(CANTIDAD_DEVOL,0))*t1.PRECIO*(1 - IFNULL(DESCUENTO, 0) / 100),2) AS 'SUBTOTAL', t3.REFERENCIA AS 'REMITO'
