...
function menu_seleccion_fact {
	...
	case $exit_status in
		...
		bash ${scr}"facturador_recepciones.sh" "RECEPCION_CONTABILIDAD" "${OPERACION}"
...
while true; do
    ...
    bash ${scr}"facturador_devoluciones.sh" "RECEPCION_CONTABILIDAD" "${OPERACION}"
...
NETO="$(cat ${temp}"temp2.cont" | grep BRUTO | awk '{print $NF}')"
...
bash ${scr}"transaccion.sh" "${PREOPER}" && dialog --msgbox "DEVOLUCION ACTUALIZADA CORRECTAMENTE" 0 0 || dialog --msgbox "OCURRIO UN ERROR EN EL ACTUALIZADO DE LA DEVOLUCION" 0 0
