...
function menu_seleccion_fact {
	...
	seleccion_comun=$(dialog \
			--backtitle "CONTABILIDAD" \
			--title "REMITO NUM. ""${REMITO}"" FECHA ""${FECHATIEMPO}""" \
			--clear \
			--cancel-label "SALIR" \
			--help-button \
	...
	case $exit_status in
		...
		bash ${scr}"facturador_recepciones.sh" "RECEPCION_CONTABILIDAD" "${PROOVEDOR}"
...
## INGRESO DE LOS DATOS DE LA OPERACION MEDIANTE LA CUAL INGRESA LA MERCANCIA

rm ${temp}"log_errores_nov.cont"


echo "PROOVEDOR">"./"${temp}"/tmp.cont"

echo "FECHA">>"./"${temp}"/tmp.cont"

echo "DATOS" >>"./"${temp}"/tmp.cont"

echo "REMITO ">>"./"${temp}"/tmp2.cont"
...
while true; do
  ...
  selection=$(dialog \
    --backtitle "CONTABILIDAD" \
    --title """NUEVA RECEPCION DE MERCADERIA""" \
    --clear \
    --cancel-label "SALIR" \
    --help-button \
    --help-label "FINALIZAR" \
    --menu "COMPLETE LOS CAMPOS POR FAVOR:" 0 0 0 "${foraneos[@]}" \
    2>&1 1>&3)
  exit_status=$?
  exec 3>&-
  case $exit_status in
  ...
  case $selection in
    ...
    "FECHA")
      ...
      buscar_dia "INGRESE LA FECHA QUE FIGURA EN EL REMITO POR FAVOR"

      echo -e "1\t${DIA_ASIGNA}" >""${temp}"selection.cont"
      echo -e "1\t"${DIA_ASIGNA}"" >>""${temp}"selection.cont"
...
PROOVEDOR="$(cat ""${temp}"tmp_num.cont" | head -1)"

FECHATIEMPO="$(cat ""${temp}"tmp_num_v.cont" | head -2 | tail -n -1)"

REMITO="$(cat ""${temp}"tmp_radio.cont" | head -1)"
...
declare -r OPERACION="$((mysql -u "${user}" --password="${pass}" --execute="START TRANSACTION;INSERT INTO "${DB}".OPERACIONES (INTERESADO,SOLICITUD,FECHA,COMPLETADO,PROVEEDOR_REFERENCIA) VALUES ('300','40','${DIA}','${DIA}',"${PROOVEDOR}");SELECT MAX(opID) FROM "${DB}".OPERACIONES;COMMIT;" | tail -n +2) || (dialog --msgbox "ERROR FATAL" 0 0 && exit 192))"

echo "INSERT INTO "${DB}".SALDO_PROOVEDOR (FECHA,REFERENCIA,PROOVEDOR,DEBE,HABER,OPERACION_ASOCIADA) VALUES ('${FECHATIEMPO}','${REMITO}',"${PROOVEDOR}","${NETO}",'0',"${OPERACION}");"  >""${temp}"grabar.cont" 
...
while read line; do
    echo "INSERT INTO "${DB}".ARMADO (ID_ARTICULO_AR,CANTIDAD_ING,OPERACION_REFERENCIA_AR) VALUES("${line}","${OPERACION}");" >>""${temp}"grabar.cont" 
done<${temp}"tmp5.cont"
...
case $? in
	...
	if test -z ${temp}"log_errores_nov.cont";then
	    ...
	    dialog --msgbox "RECEPCION INGRESADA CORRECTAMENTE" 0 0
