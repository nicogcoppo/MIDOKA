...
function cliente_comision {
    ...
    echo "UPDATE COMISIONES JOIN(OPERACIONES,CLIENTE) ON(OPERACION_REF_C=opID AND INTERESADO=clID) SET MONTO=0 WHERE clID=275 OR clID=303 OR clID=340 OR clID=337 OR clID=314 OR clID=437 OR clID=440 OR clID=443 OR clID=446;" >>${AGMYSQL}
    ...
    test -z "$(cat ${AGMYSQL} | head -1)" || (bash ${scr}"transaccion.sh" "${AGMYSQL}" && dialog --msgbox "PROCESO REALIZADO CON EXITO" 0 0 )  
