...
mysql -u "${user}" --password="${pass}" --execute="UPDATE "${DB}".SOLICITUDES SET REALIZACION='${DIA}' WHERE CLASE="${OPERACION}" AND COMIENZO='${FECHA_CARGA}' AND REALIZACION IS NULL ORDER BY scID DESC;" 
...
bash -o xtrace ${scr}"facturador_revision.sh" ""${temp}"facturar.dist"
