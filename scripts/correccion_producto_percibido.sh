...
function menu_seleccion_fact {
	...
	seleccion_comun=$(dialog \
			--backtitle "CONTABILIDAD" \
			--title "FACTURA ESPONTANEA CLIENTE: "${INTERESADO}" """ \
			--clear \
			--cancel-label "SALIR" \
			--help-button \
	...
	case $exit_status in
		...
		bash  ${scr}"facturador_espontaneo.sh" "RECEPCION_CONTABILIDAD" "${PROOVEDOR}"
...
declare -r OPERACION="$((mysql -u "${user}" --password="${pass}" --execute="START TRANSACTION;INSERT INTO "${DB}".OPERACIONES (INTERESADO,SOLICITUD,FECHA,COMPLETADO) VALUES ("""${INTERESADO}""",'49','${DIA}','${DIA}');SELECT MAX(opID) FROM "${DB}".OPERACIONES;COMMIT;" | tail -n +2) || (dialog --msgbox "ERROR FATAL" 0 0 && exit 192))"
...
while true; do
	...
	0)
	    ...
	    mysql -u "${user}" --password="${pass}" --execute="SELECT mtID,motivo_mt FROM "${DB}".MOTIVO WHERE modelo_mt="${MODELO}";" | tail -n +2 >${temp}"temp2.cont"
		...
		MOTIVO=${seleccion_comun}

		ID="$(mysql -u "${user}" --password="${pass}" --execute="SELECT artID FROM "${DB}".ARTICULOS WHERE RUBRO="${RUBRO}" AND CATEGORIA="${CATEGORIA}" AND MODELO="${MODELO}" AND MOTIVO="${MOTIVO}";" | tail -n +2)"
		
		CONTADOR="INGRESE LA CANTIDAD POR FAVOR"
		...
		while true; do
		    ...
		    CANTIDAD=$(dialog \
				   ...
				   --help-button \
				   --help-label "AYUDA" \
				   --inputbox """${CONTADOR}""" 0 0 2>&1 1>&3)
		    exit_status=$?
		    exec 3>&-

		    dialog --yesno "CANTIDAD = """${CANTIDAD}"""" 0 0 && break 1

		    CONTADOR="INGRESE NUEVAMENTE LA CANTIDAD POR FAVOR"
		done
		
		#&& test -s ""${temp}"tmp2.cont" && break || (dialog --yesno "NO EXISTE NINGUNA CATEGORIA CON ESA DESCRIPCION, DESEA VOLVER A INTENTARLO ?" 0 0 || false ) || exit 192 
		
		 test -z ${CANTIDAD} || mysql -u "${user}" --password="${pass}" --execute="INSERT INTO "${DB}".RECEPCION_CONTABILIDAD (ID_ARTICULO_RE,CANTIDAD_RE) VALUES ("${ID}","${CANTIDAD}") ;" && dialog --yesno "SEGUIR FACTURANDO UN ARTICULO SIMILAR ?" 0 0 || break
    ...
    bash -o xtrace ${scr}"facturador_espontaneo.sh" "RECEPCION_CONTABILIDAD" "${INTERESADO}"
    menu_seleccion_fact

done


NETO="$(cat ${temp}"temp2.cont" | grep NETO | awk '{print $NF}')"


rm ${temp}"log_errores_nov.cont"


(dialog --yesno "CONSIDERAR COMO ABONADA LA RECIENTE FACTURACION POR "${NETO}" ?" 0 0 && echo "INSERT INTO "${DB}".CUENTA_CORRIENTE (OPERACION_REF_CC,DEBE,HABER,DEVOLUCION,PERDIDAS) VALUES ("${OPERACION}","${NETO}","${NETO}",'0','0');" >""${temp}"grabar.cont" )|| echo "INSERT INTO "${DB}".CUENTA_CORRIENTE (OPERACION_REF_CC,DEBE,HABER,DEVOLUCION,PERDIDAS) VALUES ("${OPERACION}","${NETO}",'0','0','0');" >""${temp}"grabar.cont"
...
while read line; do
    echo "INSERT INTO "${DB}".ARMADO (OPERACION_REFERENCIA_AR,ID_ARTICULO_AR,CANTIDAD_EGR) VALUES("${OPERACION}","${line}");" >>""${temp}"grabar.cont"
done<${temp}"tmp5.cont"

bash -o xtrace ${scr}"transaccion.sh" ""${temp}"grabar.cont" && bash -o xtrace ${scr}"/facturador_espontaneo_comisiona.sh" "RECEPCION_CONTABILIDAD" "${INTERESADO}" "${OPERACION}"
