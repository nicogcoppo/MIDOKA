...
JOIN
  (SELECT 'ACTUAL' AS ACTUAL, SUM(DEBE) AS 'facturacionEspontanea'
   FROM CUENTA_CORRIENTE
   JOIN (OPERACIONES) ON(opID=OPERACION_REF_CC)
   WHERE OPERACIONES.FECHA<='2019-12-31' AND SOLICITUD=49) t22 ON(t1.ACTUAL=t22.ACTUAL)   
JOIN
...
JOIN
  (SELECT 'ACTUAL' AS ACTUAL, SUM(HABER) AS 'COBRANZA' FROM CUENTA_CORRIENTE JOIN(OPERACIONES) ON(opID=OPERACION_REF_CC) WHERE OPERACIONES.FECHA<='2019-12-31' AND OPERACIONES.FECHA>'2019-12-15') t19 ON(t1.ACTUAL=t19.ACTUAL);
