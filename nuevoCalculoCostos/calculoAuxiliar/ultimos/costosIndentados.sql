...
FROM
  (SELECT DATE_FORMAT(SOLICITUDES.REALIZACION, '%Y-%m') AS MES,
   ...
   WHERE t2.VIGENCIA IS NULL
   	 AND CLASE=9
   GROUP BY DATE_FORMAT(SOLICITUDES.REALIZACION, '%Y-%m')
   ORDER BY YEAR(SOLICITUDES.REALIZACION),
            MONTH(SOLICITUDES.REALIZACION) ASC) t1
LEFT JOIN
...
RIGHT JOIN
  (SELECT DATE_FORMAT(SOLICITUDES.REALIZACION, '%Y-%m') AS MES,
   ...
   WHERE ((VIGENCIA IS NULL
	...
	AND CLASE=9 
   GROUP BY DATE_FORMAT(SOLICITUDES.REALIZACION, '%Y-%m')) t3 ON(t1.MES=t3.MES)
LEFT JOIN
...
LEFT JOIN
  (SELECT DATE_FORMAT(SOLICITUDES.REALIZACION, '%Y-%m') AS MES,
   ...
   WHERE CLASE=9
   GROUP BY DATE_FORMAT(SOLICITUDES.REALIZACION, '%Y-%m')
   ORDER BY DATE_FORMAT(SOLICITUDES.REALIZACION, '%Y-%m') ASC) t8 ON(t1.MES=t8.MES);
