...
SELECT 'Lsup' AS 'QUINCENA',
       ...
       ROUND(t2.PAGOS+t7.FACTURACION, 0) AS 'CLIENTES',
       ROUND(t2.PAGOS+t7.FACTURACION, 0)*.1 AS 'COMISIONES FUTURAS',
       ROUND(t3.PROVEEDOR, 0) AS 'PROVEEDORs',
       ROUND(t4.CAJA, 0) AS 'CAJA',
       ROUND(t8.COSTO, 0) AS 'COSTO MERCADERIAS',
FROM
...
JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
   ...
   WHERE t2.VIGENCIA IS NULL
   	 AND CLASE=9 AND SOLICITUDES.REALIZACION<='Lsup') t5 JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
...
JOIN
  (SELECT 'ACTUAL' AS ACTUAL, SELECT SUM((CANTIDAD_AR-IF(OPERACIONES.SOLICITUD=42, CANTIDAD_ING, 0))*CANTIDAD_MINIMA*t1.PRECIO*(1 - IFNULL(DESCUENTO, 0) / 100)) AS 'COSTO'
   FROM ARMADO JOIN(PRECIOS, CATEGORIA, MODELO, CONDICION, ARTICULOS, CLIENTE, OPERACIONES, PROOVEDOR, SOLICITUDES) ON(PRECIOS.RUBRO_pr=ARTICULOS.RUBRO
                                                                                                          AND PRECIOS.CATEGORIA_pr=ARTICULOS.CATEGORIA
                                                                                                          AND PRECIOS.MODELO_pr=ARTICULOS.MODELO
                                                                                                          AND CATEGORIA.claID=ARTICULOS.CATEGORIA
...
JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
   ...
   WHERE (VIGENCIA IS NOT NULL
	AND CLASE=9 AND CANCELACION>'Linf' AND CANCELACION<='Lsup') t11 ON(t1.ACTUAL=t11.ACTUAL)	
JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
          REGISTRO AS FIJO
   FROM CONTABLES
...
JOIN
  (SELECT SELECT 'ACTUAL' AS ACTUAL, SUM(DEBE) AS 'COMPRA' FROM SALDO_PROOVEDOR JOIN(OPERACIONES) ON(opID=OPERACION_ASOCIADA) WHERE OPERACIONES.FECHA<='Lsup' AND OPERACIONES.FECHA>'Linf') t17 ON(t1.ACTUAL=t17.ACTUAL)
JOIN
  (SELECT SELECT 'ACTUAL' AS ACTUAL, SUM(HABER) AS 'PAGO' FROM SALDO_PROOVEDOR JOIN(OPERACIONES) ON(opID=OPERACION_ASOCIADA) WHERE OPERACIONES.FECHA<='Lsup' AND OPERACIONES.FECHA>'Linf') t17 ON(t1.ACTUAL=t17.ACTUAL)  
JOIN
  (SELECT SELECT 'ACTUAL' AS ACTUAL, SELECT SUM(DEBE-IFNULL(DEVOLUCION,0)) AS 'FACTURACION' FROM CUENTA_CORRIENTE JOIN(OPERACIONES,SOLICITUDES) ON(opID=OPERACION_REF_CC AND REFERENCIA=opID) WHERE SOLICITUDES.REALIZACION<='Lsup' AND SOLICITUDES.REALIZACION>'Linf' AND CLASE=9) t18 ON(t1.ACTUAL=t18.ACTUAL)
JOIN
  (SELECT SELECT 'ACTUAL' AS ACTUAL, SELECT SUM(HABER) AS 'COBRANZA' FROM CUENTA_CORRIENTE JOIN(OPERACIONES) ON(opID=OPERACION_REF_CC) WHERE OPERACIONES.FECHA<='Lsup' AND OPERACIONES.FECHA>'Linf') t19 ON(t1.ACTUAL=t19.ACTUAL);
