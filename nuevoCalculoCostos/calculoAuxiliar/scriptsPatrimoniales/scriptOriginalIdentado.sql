...
FROM
  (SELECT 'ACTUAL' AS ACTUAL,
          SUM((IFNULL(CANTIDAD_CST, 0) + IFNULL(CANTIDAD_ING, 0) + IFNULL(CANTIDAD_DEVOL, 0) - IFNULL(CANTIDAD_EGR, 0) - IFNULL(CANTIDAD_AR, 0)) * CANTIDAD_MINIMA * IF(IFNULL(PRECIO_A, 0) > IFNULL(PRECIO_B, 0), IFNULL(PRECIO_A, 0) * (1 - (DESCUENTO / 100)), IFNULL(PRECIO_B, 0) * (1 - (DESCUENTO / 100)))) AS 'STOCK'
   FROM ARMADO
   JOIN (OPERACIONES,
         RUBRO,
         CATEGORIA,
   ...
   WHERE OPERACIONES.FECHA<=LIMITACION) t1 JOIN
  (SELECT 'ACTUAL' AS ACTUAL, SUM(IFNULL(DEBE, 0) - IFNULL(HABER, 0) - IFNULL(DEVOLUCION, 0) - IFNULL(PERDIDAS, 0)) AS 'CLIENTES'
   ...
   WHERE OPERACIONES.FECHA<=LIMITACION) t2 ON(t1.ACTUAL=t2.ACTUAL) JOIN
