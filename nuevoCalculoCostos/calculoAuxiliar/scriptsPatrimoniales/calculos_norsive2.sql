...
FROM
  (SELECT 'ACTUAL' AS ACTUAL,
   ...
   WHERE OPERACIONES.FECHA<='2019-10-31') t1
JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
          SUM(-IFNULL(CANTIDAD_AR,0) * CANTIDAD_MINIMA * IF(IFNULL(PRECIO_A, 0) > IFNULL(PRECIO_B, 0), IFNULL(PRECIO_A, 0) * (1 - (DESCUENTO / 100)), IFNULL(PRECIO_B, 0) * (1 - (DESCUENTO / 100)))) AS 'armadosFacturados'
   FROM ARMADO
   JOIN (OPERACIONES,
         RUBRO,
         CATEGORIA,
   ...
   WHERE SOLICITUDES.REALIZACION<='2019-10-31' AND CLASE=9) t5 JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
   ...
   JOIN (OPERACIONES,
	 ...
	 SOLICITUDES) ON (opID=OPERACION_REFERENCIA_AR
                        ...
                        AND ARTICULOS.artID=ARMADO.ID_ARTICULO_AR
                        AND ARTICULOS.MOTIVO=MOTIVO.mtID AND opID=SOLICITUDES.REFERENCIA)
   WHERE (SOLICITUDES.COMIENZO<='2019-10-31' AND SOLICITUDES.REALIZACION IS NULL AND CLASE=8) OR (SOLICITUDES.COMIENZO<='2019-10-31' AND SOLICITUDES.REALIZACION IS NULL AND CLASE=9)) t6   
JOIN
  (SELECT 'ACTUAL' AS ACTUAL, SUM(IFNULL(DEBE, 0) - IFNULL(HABER, 0) - IFNULL(DEVOLUCION, 0) - IFNULL(PERDIDAS, 0)) AS 'CLIENTES'
   ...
   WHERE OPERACIONES.FECHA<='2019-10-31') t2 ON(t1.ACTUAL=t2.ACTUAL)
JOIN
  (SELECT 'ACTUAL' AS ACTUAL, SUM(IFNULL(DEBE, 0) - IFNULL(HABER, 0)) AS 'PROVEEDOR'
   ...
   WHERE OPERACIONES.FECHA<='2019-10-31') t3 ON(t1.ACTUAL=t3.ACTUAL)
JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
          REGISTRO AS 'CAJA',
          FECHA
   ...
   WHERE TIPO_REGISTRO=5
     AND FECHA='2019-10-31') t4 ON(t1.ACTUAL=t4.ACTUAL);

USE MIDOKA_PGC_B;
SELECT t4.FECHA AS 'UTIL',
       ROUND(t1.STOCK, 0) AS 'STOCK',
       ROUND(t5.armadosFacturados, 0) AS 'ARMADOS FACTURADOS',
       ROUND(IFNULL(t6.armados,0), 0) AS 'ARMADOS',
       ROUND(t2.CLIENTES, 0) AS 'CLIENTES',
...
FROM
  (SELECT 'ACTUAL' AS ACTUAL,
   ...
   WHERE t2.VIGENCIA IS NULL AND OPERACIONES.FECHA<='2019-10-31') t1
JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
   ...
   WHERE t2.VIGENCIA IS NULL
   	 AND CLASE=9 AND SOLICITUDES.REALIZACION<='2019-10-31') t5 JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
   ...
   LEFT JOIN PRHISTORICOS t2 ON(t1.RUBRO=t2.RUBRO
                                ...
                                AND t2.VIGENCIA <='2019-10-31')
   WHERE t2.VIGENCIA IS NULL
   	 AND ((SOLICITUDES.COMIENZO<='2019-10-31' AND SOLICITUDES.REALIZACION IS NULL AND CLASE=8) OR (SOLICITUDES.COMIENZO<='2019-10-31' AND SOLICITUDES.REALIZACION IS NULL AND CLASE=9))) t6   
JOIN
  (SELECT 'ACTUAL' AS ACTUAL, SUM(IFNULL(DEBE, 0) - IFNULL(HABER, 0) - IFNULL(DEVOLUCION, 0) - IFNULL(PERDIDAS, 0)) AS 'CLIENTES'
   FROM CUENTA_CORRIENTE
   JOIN (OPERACIONES) ON(opID=OPERACION_REF_CC)
