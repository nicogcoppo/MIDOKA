...
SELECT 'delorean' AS ACTUAL,
          (IFNULL(CANTIDAD_CST, 0) + IFNULL(CANTIDAD_ING, 0) + IFNULL(CANTIDAD_DEVOL, 0) - IFNULL(CANTIDAD_EGR, 0)) AS 'CANTIDADES',
	  mod_md AS 'MODELO',
   ...
   JOIN (OPERACIONES,
	 ...
	 CLIENTE,
         ARTICULOS) ON (opID=OPERACION_REFERENCIA_AR
                        ...
                        AND ARTICULOS.MOTIVO=MOTIVO.mtID AND INTERESADO=clID)
   WHERE OPERACIONES.FECHA<='2019-12-31' ORDER BY opID,mod_md LIMIT 25;
...
SELECT 'util' AS ACTUAL,
   ...
   WHERE t2.VIGENCIA IS NULL ORDER BY opID,mod_md LIMIT 25;
