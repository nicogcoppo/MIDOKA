...
FROM
  (SELECT 'ACTUAL' AS ACTUAL,
   ...
   WHERE OPERACIONES.FECHA<='LIMITACION') t1
JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
   ...
   WHERE SOLICITUDES.REALIZACION<='LIMITACION' AND CLASE=9) t5 JOIN
  (SELECT 'ACTUAL' AS ACTUAL,
   ...
   JOIN (OPERACIONES,
	 ...
	 SOLICITUDES) ON (opID=OPERACION_REFERENCIA_AR
                        ...
                        AND ARTICULOS.artID=ARMADO.ID_ARTICULO_AR
                        AND ARTICULOS.MOTIVO=MOTIVO.mtID AND opID=SOLICITUDES.REFERENCIA)
   WHERE (SOLICITUDES.COMIENZO<=NOW() AND SOLICITUDES.REALIZACION IS NULL AND CLASE=8) OR (SOLICITUDES.COMIENZO<=NOW() AND SOLICITUDES.REALIZACION IS NULL AND CLASE=9)) t6   
JOIN
