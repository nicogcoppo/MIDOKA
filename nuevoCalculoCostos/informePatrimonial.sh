...
mysql -u "${user}" --password="${pass}" --execute="USE ${DB};SELECT FECHA FROM CONTABLES WHERE TIPO_REGISTRO=5 AND FECHA >='2019-01-01' ORDER BY FECHA ASC;" | tail -n +2 > /tmp/patrimonio.dat
...
for ((i=1;i<fechasTotal;i++)) ; do
    ...
    cat ./calculoPatrimonial.sql | sed "s/Linf/${Linf}/g" | sed "s/Lsup/${Lsup}/g" >/tmp/calculoPatrimonial.sql
